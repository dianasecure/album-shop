...
export default function RemoveAlbumPage(){
  ...
  const handleRemove = (albumId) => {
    removeAlbum(albumId);  // Call removeAlbum with the specific album's id
  };
  ...
  return(
    <div className="p-6 bg-orange-50 min-h-screen">
      ...
      <div className="text-center mb-6">
        <h1 className="text-2xl font-bold text-stone-950">Choose album to remove</h1>
      </div>
        ...
        {albums.map((album) => (
          <div key={album.id} >
            ...
            <div className="p-4 bg-fuchsia-100 rounded-md w-48 flex flex-col m-2 mt-0">
                <button type="button" onClick={() => handleRemove(album.id)} className="bg-red-500 text-white px-2 py-1 rounded hover:bg-red-600">
                  Remove
                </button>
            </div>
          </div>
